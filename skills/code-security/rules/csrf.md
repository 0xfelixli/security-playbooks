...
- API endpoints using **stateless authentication** (Bearer JWT, API key in header) — CSRF requires ambient credentials (cookies/session). JWT in `Authorization` header is not sent by browser cross-origin automatically.
- `@csrf_exempt` on endpoints that only accept `Content-Type: application/json` — browsers don't auto-send JSON cross-origin without CORS preflight
- `SameSite=Strict` or `SameSite=Lax` cookies already configured — these mitigate CSRF without an explicit token for most cases
- Read-only (`GET`/`HEAD`/`OPTIONS`) endpoints — CSRF targets state-changing operations
