...
detect:
  files: ["*.yaml", "*.yml"]
  content: ["apiVersion:", "kind: Pod", "kind: Deployment", "kind: DaemonSet"]
---
...
- `privileged: true` on a **DaemonSet that manages host networking** (e.g., CNI plugin, node exporter) — intentional; document the business justification
- `hostNetwork: true` on a **node-level monitoring agent** that needs raw packet access — same; flag for documentation, not remediation
- `hostPID: true` on a **debugging/tracing tool** (e.g., `strace`, `bpftrace`) deployed to a locked-down namespace — acceptable for ops tooling with restricted RBAC
- Kubernetes `Secret` resource — storing data in a `Secret` object is the correct K8s pattern; the finding is unencrypted `etcd` (cluster-level config) or secrets in `ConfigMap`/`env` literals in pod specs
- `allowPrivilegeEscalation: true` on a container that **drops all capabilities** and runs as a fixed non-root UID — escalation has no capabilities to gain; low residual risk
