...
- `gets_s` / `fgets` with explicit buffer size bound — safe bounded-read functions
- `strcpy` / `sprintf` where **destination buffer is provably large enough** (hardcoded size ≥ source maximum) and source is not user-controlled
- `unsafe` block in Rust covering only a FFI call with no user-controlled pointer arithmetic and a `// SAFETY:` invariant comment
- `Array.unsafe_get` in OCaml inside a **bounds-checked wrapper** that asserts index < length before delegating
- C `strlen` on a **string literal** (`strlen("fixed")`) — no overflow risk
