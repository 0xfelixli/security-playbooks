...
- `os.path.join(BASE_DIR, filename)` where `BASE_DIR` is a hardcoded constant **and** the code subsequently checks that the resolved path starts with `BASE_DIR` (containment check present)
- File access using a hardcoded path with no user-supplied component: `open("/etc/app/config.yaml")`
- File path taken from server-side config/database, not from user input
- Static file serving from a fixed directory where the web framework resolves paths safely (e.g., FastAPI `StaticFiles`, Express `express.static`)
