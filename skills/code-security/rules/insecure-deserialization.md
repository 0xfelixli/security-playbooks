...
- `json.loads()` / `JSON.parse()` / `json_decode()` — JSON only returns primitives; no code execution possible
- `pickle.loads()` on data from **trusted internal storage** (e.g., same-process cache, signed+verified payload) — still flag if signature verification is missing
- `yaml.safe_load()` / `YAML::safe_load` — safe loaders disable arbitrary object construction
- Protobuf / MessagePack / Avro deserialization — these are schema-constrained binary formats without arbitrary object construction
- `ast.literal_eval()` — evaluates only Python literals, not arbitrary objects
