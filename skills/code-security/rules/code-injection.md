...
- `ast.literal_eval()` — evaluates only Python literals (strings, numbers, tuples, lists, dicts), not arbitrary code
- `JSON.parse()` — parses JSON data, not executable JavaScript
- `eval()` / `exec()` with a **hardcoded string literal** (no user input): `eval("1 + 1")`
- Template engines rendering variables into HTML (Jinja2, Django templates) — that is XSS, not code injection; check `rules/xss.md` instead
