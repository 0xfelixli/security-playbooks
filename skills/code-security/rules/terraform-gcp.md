...
detect:
  files: ["*.tf", "*.tfvars"]
  content: ["google_", "provider \"google\""]
---
...
- **公开静态网站 Bucket**：`google_storage_bucket_iam_member` 中 `member = "allUsers"` 且 `role = "roles/storage.objectViewer"` 用于托管静态资源，是预期公开行为。
- **公网 Cloud Run 服务**：`google_cloud_run_service_iam_member` 设置 `allUsers` + `roles/run.invoker` 用于对外提供 API，是合理公开配置；需结合 IAP 或鉴权代码判断，不应直接报高危。
- **GKE 公网集群（受控访问）**：`master_authorized_networks_config` 限定了具体 CIDR 的公网 GKE master，不等同于完全开放；仅在 `cidr_block = "0.0.0.0/0"` 时才是问题。
- **Google 默认加密**：GCS/BigQuery/Pub-Sub 不配置 `kms_key_name` 时 GCP 使用 Google 托管密钥默认加密，仅在合规要求 CMEK 时才构成风险，不应泛报。
- **Dev/test 环境**：文件路径或模块名含 `dev`/`test`/`sandbox` 时，降级处理，不作为高危 finding。
