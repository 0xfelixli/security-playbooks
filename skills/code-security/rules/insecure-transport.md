...
- `verify=False` / `InsecureSkipVerify: true` in **test fixtures or CI against a localhost mock** — flag only if it can reach production hosts
- Self-signed certificate with a **custom CA pinned** via `verify="/path/to/ca.crt"` — chain is verified, just not against public roots
- Internal service-to-service traffic over **mutual TLS (mTLS)** on a private network — both sides authenticated; acceptable when network is already trusted
- HTTP (not HTTPS) for **localhost-only** traffic (`127.0.0.1`, `::1`) — no network exposure
- `http://` URLs in test configurations that never execute against real endpoints
