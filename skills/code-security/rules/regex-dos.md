...
- Regex matched against **length-bounded input** (e.g., validated ≤ 64 chars before matching) — catastrophic backtracking requires long input
- Regex engine is **RE2** (Go's `regexp`, Python `re2` package) — guarantees linear-time matching regardless of pattern
- Pattern uses only **non-backtracking constructs** (no nested quantifiers, no alternation with overlap): anchored, fixed-length patterns like `^\d{4}-\d{2}-\d{2}$`
- Regex executed with a **timeout** that interrupts runaway evaluation
- Pattern applied only to **server-internal data** never derived from user input
