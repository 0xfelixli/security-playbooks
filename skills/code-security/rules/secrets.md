...
- Values read from `os.environ`, `os.getenv`, `process.env`, `config.get()`, or a secret manager — these are the correct patterns
- Placeholder/example strings that are obviously not real: `"CHANGE_ME"`, `"your-api-key-here"`, `"<SECRET>"`, `"xxx"`, `"test_"`
- Keys in comments only (not assigned to a variable or used in code)
- Intentionally public API keys such as Google Analytics / Segment write keys — these are designed to be public
- Keys in test fixtures pointing to obviously fake/sandbox endpoints (`stripe_test_`, `sandbox`, `example.com`)
