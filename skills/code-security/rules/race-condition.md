...
> **⚠ 评级说明**：本文件覆盖通用竞态条件（临时文件、文件系统 TOCTOU），impact 为 **MEDIUM**。
> 如果 TOCTOU 发生在**支付、提现、余额扣减、审批状态**等业务关键流程中，
> impact 应升级为 **CRITICAL** — 参考 `rules/business-logic.md`（竞态条件 + 原子性 section）。
...
- `select_for_update()` / `FOR UPDATE` applied before check and update — row-level lock eliminates TOCTOU
- Atomic SQL: `UPDATE ... SET balance = balance - amount WHERE balance >= amount` — single statement, no window
- Optimistic locking with version field: update rejected if `version` has changed — correct concurrency control
- Operation is **idempotent** and the result of a race is identical to sequential execution — no exploitable outcome
- Counter incremented with `F()` expression (`balance=F('balance') - amount`) inside a transaction — atomic at DB level
