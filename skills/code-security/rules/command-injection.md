...
- `subprocess.run(["cmd", arg1, arg2])` — list form, never shell-interpolated; OS escapes each argument
- `subprocess.run(..., shell=False)` (explicit) — equivalent to list form
- Arguments are **hardcoded string literals** with no user-controlled data — no injection surface
- `shlex.split()` used to tokenize, followed by list-form `subprocess` — correct pattern when command is fixed and only safe tokens are inserted
- `os.system` / `subprocess.run` called inside tests with controlled fixture values
