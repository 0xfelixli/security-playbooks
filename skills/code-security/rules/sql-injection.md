...
- Parameterized queries that happen to use string formatting *for the parameter placeholder*, not the value: `cursor.execute("SELECT * FROM t WHERE id = %s", [user_id])` — the `%s` is a placeholder, not concatenation
- SQL in test/migration files with hardcoded, non-user-supplied values
- ORM methods like `Model.objects.filter(id=user_id)` where the ORM handles parameterization automatically
- `text()` with bound parameters: `db.execute(text("SELECT * FROM t WHERE id = :id"), {"id": user_id})`
