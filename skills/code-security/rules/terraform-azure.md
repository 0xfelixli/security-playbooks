...
detect:
  files: ["*.tf", "*.tfvars"]
  content: ["azurerm_", "provider \"azurerm\""]
---
...
- **静态网站存储**：`allow_nested_items_to_be_public = true` 配合 `azurerm_storage_blob` 用于托管公开静态内容（如 CDN 源站），属于预期行为。
- **公网容器实例**：`ip_address_type = "public"` 用于对外暴露服务的容器组，是合理的公网配置；需结合端口和网络规则判断，不应直接报高危。
- **TLS 1.0/1.1 的遗留兼容注释**：`minimum_tls_version` 设置在注释中明确说明是为了兼容遗留客户端，且已有迁移计划时，降级为 MEDIUM。
- **Key Vault 软删除 0 天**：仅在明确标注 `lifecycle { prevent_destroy = true }` 的测试用途 vault 中，purge protection 可不强制要求，不作为 finding。
