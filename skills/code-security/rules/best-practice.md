...
- `open()` used inside a `with` statement — correct; file is guaranteed to close
- `tempfile.NamedTemporaryFile()` / `tempfile.mkstemp()` — safe temporary file creation; no TOCTOU
- File opened by path where the path is **hardcoded or admin-configured**, not user-supplied
- `eval()` used in a test harness on **hardcoded string literals** — no user input reaches it
