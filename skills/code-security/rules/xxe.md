...
- `defusedxml` used instead of `lxml`/`xml.etree` — defusedxml disables external entities by default
- XML parser explicitly configured with `resolve_entities=False` / `no_network=True`
- JSON or YAML parsed (not XML) — XXE is XML-specific
- Internal XML documents with no user-supplied content
