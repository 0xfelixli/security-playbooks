...
- MD5 / SHA-1 used as **non-cryptographic checksums** (cache keys, content deduplication, ETag generation) where collision resistance is not a security requirement
- `random.random()` / `Math.random()` used for non-security purposes (UI animations, shuffling display order, non-secret sampling)
- SHA-256 or stronger used for password hashing **without** a salt — still flag this; SHA-256 is not a password hash. Use bcrypt/argon2 instead.
