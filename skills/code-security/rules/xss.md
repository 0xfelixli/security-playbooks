...
- `dangerouslySetInnerHTML` where `__html` is sourced from a trusted CMS/static content that never accepts user input
- `innerHTML` in test files (jest, testing-library) where the HTML is a hardcoded fixture
- Template engines (Jinja2, Django templates, React JSX) rendering variables without `|safe` / `mark_safe` — auto-escaping is the secure default
- `v-html` / `[innerHTML]` bound to a constant string literal, not a variable
