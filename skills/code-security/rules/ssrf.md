...
- HTTP call to a fully hardcoded URL with no user-supplied component: `requests.get("https://api.stripe.com/v1/charges")`
- URL read from server-side configuration (env var, config file) that is never exposed to user input
- Outbound URL validated against a strict allowlist *before* the request is made (not after)
- Internal service-to-service calls where the target URL is compiled-in and cannot be influenced by external input
