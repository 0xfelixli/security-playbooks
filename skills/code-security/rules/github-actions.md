...
- **`actions/*` / `github/*` 未 SHA 固定**：GitHub 官方 actions（`actions/checkout`、`actions/setup-node` 等）和本地 actions（`./.github/actions/*`）不需要 SHA 固定，规则文件已说明。
- **`if:` 条件中的 context 插值**：`${{ github.event_name == 'push' }}` 这类布尔判断用在 `if:` 而非 `run:` 中不存在注入风险。
- **`env:` 中使用 context 变量**：`PR_TITLE: ${{ github.event.pull_request.title }}` 赋给环境变量后在 `run:` 里通过 `$PR_TITLE` 引用，是正确的防注入写法，不应报告。
- **`pull_request_target` 无 checkout 且无 secrets**：使用 `pull_request_target` 触发器但 workflow 内无 `actions/checkout` 且无 `secrets.*` 引用时，不构成 pwn-request 风险。
- **内部仓库私有 action 未 SHA 固定**：同组织内自有 action 使用分支引用（如 `@main`）风险可控，不作为高危 finding。
